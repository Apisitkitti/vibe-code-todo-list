...
import { clearedFiltersUrl, TODOS_PATH, todosUrl } from "@/lib/todosUrl";
...
describe("clearedFiltersUrl", () => {
  ...
  */
  it("drops the filters and keeps the view", () => {
    expect(clearedFiltersUrl("board")).toContain("view=board");
    expect(clearedFiltersUrl("board")).not.toContain("status=");
    expect(clearedFiltersUrl("list")).toBe(TODOS_PATH);
  });
});
