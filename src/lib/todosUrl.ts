 ...
 * leave the board they are looking at. Emptying the whole query string would
 * have done both, and the second half would have looked like the app losing
 * its place. Because this is the filters alone, every writer that lays it over
 * the current state keeps the view for free.
 *
 * Kept as the tuple rather than as a finished URL so the one caller that
 * clears can push it through the same guard as every other change
 * (`useTodosUrlSync`). A `clearedFiltersUrl` helper would have to write the URL
 * itself, which is exactly the second unguarded writer this app has already
 * been bitten by.
 */
export const CLEARED_FILTERS: TodoListFilters = {
